...
class ProductPostgresRepository(ProductsRepositoryInterface):
    ...
    def _list_products(self, filter: dict, page: int, page_size: int) -> list[Product]:
        skip = (page - 1) * page_size
        query = select(ProductModel).filter_by(**filter).offset(skip).limit(page_size)
        with self.db_session() as session:
    ...
    def _update_product(self, id: int, **kwargs) -> Product:
        ...
        with self.db_session() as session:
            result = (
                session.query(ProductModel).filter(ProductModel.id == id).update(kwargs)
            )
            session.commit()
        if result:
